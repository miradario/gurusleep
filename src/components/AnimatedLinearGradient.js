...
class AnimatedGradient extends Component {
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    if (nextProps.colors !== this.props.colors) {
      ...
      this.state.animatedColors.forEach(animatedColor =>
  ...
  interpolateColors = ({ colors }) => {
    return this.state.animatedColors.map((animatedColor, i) => {
      return animatedColor.interpolate({
        inputRange: [0, 1],
        outputRange: [this.props.colors[i], colors[i] || "transparent"]
      });
    });
  };
  ...
  render() {
    ...
    const colorProps = this.interpolatedColors.reduce(
      (obj, interpolatedColor, i) => {
